...
{
    public partial class UcReporteMetodosPago : UserControl
    {
        private readonly DateTime _desde, _hasta;
        private readonly ReportePropietarioRepository _repo = new();
        private DataGridView grid;
        private Label lblTotal, lblCantidad;

        public UcReporteMetodosPago(DateTime desde, DateTime hasta)
        {
        ...
        private void BuildUi()
        {
            ...
            header.Controls.Add(new Label { Text = "Ingresos por Método de Pago", AutoSize = true, ForeColor = Color.White, Font = new Font("Segoe UI", 12, FontStyle.Bold) });
            ...
            var c1 = UiTheme.KpiCard("Total Ingresos", out lblTotal, UiTheme.Primary);
            var c2 = UiTheme.KpiCard("Pagos", out lblCantidad, UiTheme.Success);
            ...
            export.Click += (s, e) => GridExporter.ExportToCsv(grid, "metodos_pago.csv");
        ...
        {
            var data = _repo.ObtenerIngresosPorMetodo(_desde, _hasta);
            grid.DataSource = data;
            lblTotal.Text = data.Sum(x => x.Total).ToString("C2");
            lblCantidad.Text = data.Sum(x => x.Cantidad).ToString();
        }
    }
}
